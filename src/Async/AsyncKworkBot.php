...
/**
 * WebSocket bot with async API helpers for handling incoming Kwork messages.
 */
class AsyncKworkBot extends AsyncKworkClient
{
     ...
     */
    public function runAsync(): PromiseInterface
    {
        return $this->getChannelAsync()->then(function (string $channel): void {
            $this->runWithChannel($channel);
        });
    }
    ...
    {
        $channel = $this->getChannelAsync()->wait();
        yield from $this->runWithChannelGenerator($channel);
    }

    private function runWithChannel(string $channel): void
    {
        foreach ($this->runWithChannelGenerator($channel) as $message) {
            $this->processMessage($message);
        }
    }
     ...
     * @return \Generator<int, Message, mixed, void>
     */
    private function runWithChannelGenerator(string $channel): \Generator
    {
    ...
    public function checkIsFirstMessage(Message $message): bool
    {
        ...
        [$pageMessages, $paging] = $this->getDialogWithUserPageAsync($username, 1)->wait();
        ...
        if ($pages === null) {
            ...
            $dialogMessages = $this->getDialogWithUserAsync($username)->wait();
    ...
    private function getUsernameForUserId(int $userId): ?string
    {
        ...
        $page = 1;
        while (true) {
            $dialogs = $this->getDialogsPageAsync($page)->wait();
            if ($dialogs === []) {
                return null;
            }
